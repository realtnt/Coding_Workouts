...
def cell_clicked( event_data ):
    ...
    text_player_info.value = "Player " + str(player_selected) + " " + random.choice(player_phrase)
...
for x in range(0, 3):
    for y in range(0, 3):
        text.append( Text(board, grid=[x, y], height=cell_height, width=cell_width, bg=cell_bg_colour) )
...
# reset board button
btn_reset = PushButton(app, text="Reset Board", command=reset_board, width="fill")
btn_reset.disable()
...
btn_restart_match = PushButton(messages, text="Restart\nMatch", width=13, height = 2, grid=[1, 0], command = restart_match)
...
text_player_info = Text(app, text="Player 1 " + random.choice(player_phrase), bg=player_1_colour, width = "fill", align = "bottom")
